...
@router.post(
    ...
    summary="Process a single PDF file with misspell correction",
    description=(
        "Same as /process but adds a misspell-correction step after OCR formatting. "
        "Each page's text is sent to the Dify misspell-correction app before being "
        "embedded as an invisible text layer."
    ),
)
async def process_pdf_with_correction(
    file: UploadFile = File(..., description="PDF file to process"),
    target_path: str = Form(..., description="Absolute directory path where the output PDF will be saved"),
) -> dict:
    if not file.filename or not file.filename.lower().endswith(".pdf"):
    ...
    try:
        ...
        if pdf_type == "NORMAL_TEXT":
            ...
            # Misspell correction — one API call per page
            corrected_pages = [
                correct_misspell(page, settings.MISSPELL_API_KEY, settings.MISSPELL_BASE_URL)
    ...
    return {
        ...
        "output": output_path,
        "ocr_result": formatted_pages,
        "corrected_result": corrected_pages,
    }
